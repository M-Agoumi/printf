...
/*   Updated: 2019/04/29 02:17:34 by magoumi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_longlongint(char *flag, t_data *var, t_flags *x)
{
	...
	nbr = flag[len] == 'x' ? to_hexa(nb) : to_hex(nb);
	if (x->pnd && nb)
		nbr = ft_strjoin("0x", nbr);
	if (x->plus && !x->piont && !(x->prec && x->width) && !(x->width && !x->moin && !x->zero) && !(x->plus && x->prec))
		ft_putcharlen('+', var);
	if (x->prec && x->width)
...
static void	ft_normalint(char *flag, t_data *var, t_flags *x)
{
	...
	nbr = flag[len] == 'x' ? to_hexa(nb) : to_hex(nb);
	if (x->pnd && nb && (x->width && x->moin) && !x->zero && (x->moin && !x->width) && (!x->width))
	{
		nbr = ft_strjoin("0x", nbr);
	}
	last = ft_strlen(flag);
